---
title: 01 — History
project: P101581 Boitsfort Bardage
project_number: P101581
location: Boitsfort (Brussels), Station, Line 161
phase: suivi d'exécution
role: Ingénieur structures — revue technique bardage ventilé
client: Infrabel
contractor: Equans / De Raedt Ivan / Etanco-Marcovis
status: active
tags: [P101581, boitsfort, history]
updated: 2026-06-04
---

# P101581 — History

Short factual record. Reverse chronological. No reasoning — only what happened and when.

---

- 2026-06-04 — Vault restructured to standard 00–06 format per _Projects_Rules.md.
- 2026-06-04 — Six blocking findings (A–F) synthesized in [[Projects/101581 Boitsfort/06 — Technical Reviews]]. Formal response to Equans in preparation.
- 2026-05-04 — TF-2100 v4.0, TF-2101 v2.0, UP-4044 v2.0 received via Hermes. Technical review initiated by Nicolas.
- 2026-04 — **NC03 non-conformité registered.** Works stopped on NU5 pilot — workmanship defects (latte distances, enduit, holes, screws hammered).
- 2026-03-11 — UP-4044 v2.0 (execution procedure) submitted by Equans.
- 2026-02-26 — Pull-out test DER 262602-1 signed by Thomas Gall (Aubergenville).
- 2026-02-18 — Pull-out test DER 262602-1 completed: 15 FM-X3 specimens at 70 mm embedment in concrete (grade unknown). All failed in plug slippage. N_Rd,s = 846 N (84.6 daN).
- 2025-10 — Project handover: Dimitri → Raphaëlle Ciuch (Senior Design Engineer, T-STR). Nicolas Papalabros assigned for structural review. Prior reviewer: Arnaud Carpent.

---

*Entries are never deleted or edited after writing. History is append-only.*

Links: [[00 — Overview]] · [[05 — Journal]]
